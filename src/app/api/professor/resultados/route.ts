...
export async function GET() {
  ...
  const { data: provas, error } = await admin.from('provas').select('id,titulo,criado_em,disciplinas(nome,cor_hex),resultados_omr(id,nota,acertos,erros,nao_respondidas,corrigido_em,aluno_id)').eq('professor_id', user.id).order('criado_em', { ascending: false })
  ...
  const resultado = (provas??[]).map(prova => {
    ...
    const maior = notas.length>0 ? Math.max(...notas) : null
    const menor = notas.length>0 ? Math.min(...notas) : null
    const aprovados = notas.filter(n=>n>=6).length
    return { id:prova.id, titulo:prova.titulo, criado_em:prova.criado_em, disciplina:prova.disciplinas, total_corrigidos:resultados.length, media, maior_nota:maior, menor_nota:menor, aprovados, reprovados:notas.length-aprovados, resultados:resultados.map(r=>({id:r.id,nota:r.nota,acertos:r.acertos,erros:r.erros,nao_respondidas:r.nao_respondidas,corrigido_em:r.corrigido_em,aluno:r.aluno_id?alunosMap[r.aluno_id]??null:null})) }
  })
