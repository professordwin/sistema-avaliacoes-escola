...
export default function ImprimirProva() {
  ...
  const [data, setData] = useState<ProvaData | null>(
    null
  )
  ...
  const [mostrarGabarito, setMostrarGabarito] =
    useState(false)

  const [modoImpressao, setModoImpressao] =
    useState<'turma' | 'individual'>(
      'individual'
    )
  ...
  useEffect(() => {
    async function carregarProva() {
      try {
        ...
        const resultado =
          (await response.json()) as ProvaData
      ...
      <div className="mt-8 border-t-2 border-gray-800 pt-4">
        ...
        <div className="grid grid-cols-5 gap-2">
          {questoes.map((_, i) => (
            ...
            >
              <span className="text-xs font-bold text-gray-700 w-4">
                {i + 1}
              </span>
